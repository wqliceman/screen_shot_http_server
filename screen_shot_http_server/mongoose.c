...
#define MG_CTL_MSG_MESSAGE_SIZE 8192   //设置控制消息的最大长度
...
#ifdef _WIN32
/* Return value is the same as for MultiByteToWideChar. */
...
#if MG_ENABLE_MQTT  //如果打开了mqtt功能
struct mg_mqtt_message;

#define MG_MQTT_ERROR_INCOMPLETE_MSG -1 //不完整的消息
#define MG_MQTT_ERROR_MALFORMED_MSG -2 //畸形的消息

MG_INTERNAL int parse_mqtt(struct mbuf *io, struct mg_mqtt_message *mm);//从缓冲区中解析mqtt消息
#endif
...
MG_INTERNAL int mg_sntp_parse_reply(const char *buf, int len,
                                    struct mg_sntp_message *msg);//网络时间协议
#endif
...
struct dirent *readdir(DIR *d)//读文件夹
{
	struct win32_dir *dir = (struct win32_dir *) d;
...
size_t mbuf_insert(struct mbuf *a, size_t off, const void *buf, size_t len)//向缓存区的某个位置插入len长度的数据
{
	char *p = NULL;
	...
	/* check overflow */
	if (~(size_t)0 - (size_t)a->buf < len) return 0;//这个检查溢出是不是没什么用啊

	if (a->len + len <= a->size)
	{
		memmove(a->buf + off + len, a->buf + off, a->len - off);//为什么会有这么一句
		if (buf != NULL)
		{
...
{
	return mbuf_insert(a, a->len, buf, len);//插入len长度的数据
}
...
const char *mg_strstr(const struct mg_str haystack,
	const struct mg_str needle)//字符串str2是否是str1的子串
{
	size_t i;
...
int mg_avprintf(char **buf, size_t size, const char *fmt, va_list ap)
{
	...
	if (len < 0)
	{
		...
		*buf = NULL; /* LCOV_EXCL_START */
		while (len < 0)
		{
			MG_FREE(*buf);//这个已经赋值为0，然后再free，这个难道不会出错吗？
			if (size == 0)
			{
				size = 5;
			}
...
struct mg_str mg_next_comma_list_entry_n(struct mg_str list, struct mg_str *val, struct mg_str *eq_val)//先找',',再找'='符号的两边
{
	if (list.len == 0)//list的长度为0，就是list没有任何内容
	{
		/* End of the list */
		list = mg_mk_str(NULL);
	}
	else
	{
		...
		*val = list;
		...
		if ((chr = mg_strchr(*val, ',')) != NULL)//找出','这个符号在val中的位置
		{
...
MG_INTERNAL void mg_add_conn(struct mg_mgr *mgr, struct mg_connection *c)
{
	...
	mgr->active_connections = c;//插入到第一个位置
	c->prev = NULL;
...
MG_INTERNAL void mg_call(struct mg_connection *nc, mg_event_handler_t ev_handler, void *user_data, int ev, void *ev_data)
{
	if (ev_handler == NULL)//如果用户没有定义事件处理函数 下面的两种可能都有
	{
		/*
		 * If protocol handler is specified, call it. Otherwise, call user-specified
		 * event handler.
		 */
		ev_handler = nc->proto_handler ? nc->proto_handler : nc->handler;//proto_handler指向的是mg_http_handler，handler指向的是handler
	}//nc->proto_handler是系统定义的，nc->handler是用户自定义的
	if (ev != MG_EV_POLL)//这个事件是由系统处理的事件
	{
		DBG(("%p %s ev=%d ev_data=%p flags=0x%lx rmbl=%d smbl=%d", nc,
			ev_handler == nc->handler ? "user" : "proto", ev, ev_data, nc->flags,
			(int)nc->recv_mbuf.len, (int)nc->send_mbuf.len));//这个poll应该
	}
	...
	{
		mg_hexdump_connection(nc, nc->mgr->hexdump_file, NULL, 0, ev);//这个是这个连接的dump文件
	}
#endif
	if (ev_handler != NULL)//事件处理函数
	{
		unsigned long flags_before = nc->flags;
		ev_handler(nc, ev, ev_data MG_UD_ARG(user_data));//调用事件处理函数进行处理
...
MG_INTERNAL void mg_timer(struct mg_connection *c, double now)//定时器事件
{
	if (c->ev_timer_time > 0 && now >= c->ev_timer_time)//这边是发现超时，就调用超时的事件
	{
...
MG_INTERNAL size_t recv_avail_size(struct mg_connection *conn, size_t max)//接受缓存区可以的空余缓存大小
{
	size_t avail;
...
int mg_if_poll(struct mg_connection *nc, double now)//这边的函数意思是系统的poll是触发定时器事件，然后是poll事件，这边都是协议已经默认的事件，不是用户自定义的
{
	...
	mg_timer(nc, now);
	{
		time_t now_t = (time_t)now;//表示的是当前时间
		mg_call(nc, NULL, nc->user_data, MG_EV_POLL, &now_t);//这个是对接收到的数据进行分析处理 这边是触发poll事件，这边触发的是系统的，然后在系统里再触发用户自定义的
	}
	return 1;
}
...
void mg_mgr_init(struct mg_mgr *m, void *user_data)//初始化这个管理器
{
	struct mg_mgr_init_opts opts;//这个是关于初始化的一些参数
...
{
	memset(m, 0, sizeof(*m));//将代表mg_mgr的这个内存都清空
#if MG_ENABLE_BROADCAST
	m->ctl[0] = m->ctl[1] = INVALID_SOCKET;//初始化两个配对的socket。这个是在允许广播的情况下会有这两个socket
#endif
	...
	{
		WSADATA data;
		WSAStartup(MAKEWORD(2, 2), &data);//初始化网络库
	}
...

	{
		...
		m->ifaces = (struct mg_iface **) MG_MALLOC(sizeof(*m->ifaces) * opts.num_ifaces);//这个ifaces是指针的指针，表示的应该是mg_iface *指针的数组
		for (i = 0; i < opts.num_ifaces; i++)
		{
			m->ifaces[i] = mg_if_create_iface(opts.ifaces[i], m);//这边是创建一个iface
			m->ifaces[i]->vtable->init(m->ifaces[i]);//处理函数集合的初始化
		}
	}
...
int mg_mgr_poll(struct mg_mgr *m, int timeout_ms)
{
	...
	for (i = 0; i < m->num_ifaces; i++)//遍历这个manager里面的所有iface，每个iface代表一种连接协议
	{
		m->ifaces[i]->vtable->poll(m->ifaces[i], timeout_ms);//这个poll里转定义到mg_socket_if_poll这个函数
	}
...
{
	struct mg_connection *conn;//分配一个结构，初始化相关参数

	if ((conn = (struct mg_connection *) MG_CALLOC(1, sizeof(*conn))) != NULL)
	{
		conn->sock = INVALID_SOCKET;
		conn->handler = callback;//也就是用户自定义的相关处理函数最终是放在mg_connection的这个变量里
		conn->mgr = mgr;
		conn->last_io_time = (time_t)mg_time();
		conn->iface =
			(opts.iface != NULL ? opts.iface : mgr->ifaces[MG_MAIN_IFACE]);//获取 MG_MAIN_IFACE
...
MG_INTERNAL struct mg_connection *mg_create_connection(struct mg_mgr *mgr, mg_event_handler_t callback, struct mg_add_sock_opts opts)//创建一个连接
{
	struct mg_connection *conn = mg_create_connection_base(mgr, callback, opts);

	if (conn != NULL && !conn->iface->vtable->create_conn(conn))//实际的连接函数
	{
		MG_FREE(conn);
		conn = NULL;
	}
...
struct mg_connection *mg_if_accept_new_conn(struct mg_connection *lc)//这边的这个连接时用来监听http连接请求的链接
{
...
void mg_send(struct mg_connection *nc, const void *buf, int len)//发送数据,这边只是将数据追加到缓冲区，并更新时间
{
...
static int mg_do_recv(struct mg_connection *nc)//具体的接收数据
{
	int res = 0;
	char *buf = NULL;
	...
	{
		mbuf_resize(&nc->recv_mbuf, nc->recv_mbuf.len + len);//扩大内存
	}
...
void mg_if_can_recv_cb(struct mg_connection *nc)//这个是接收到有数据可以读的时候的回调
{
	mg_do_recv(nc);
}
...
#endif
	{
		n = nc->iface->vtable->tcp_recv(nc, buf, len);//这边在websocket的时候报错
		DBG(("%p <- %d bytes", nc, n));
	}
	if (n > 0)
	{
		...
		mg_call(nc, NULL, nc->user_data, MG_EV_RECV, &n);//这个mg_call就会调用系统自定义，以及用户自己定义的事件处理函数
	}
	else if (n < 0)
	{
		nc->flags |= MG_F_CLOSE_IMMEDIATELY;
...
void mg_if_can_send_cb(struct mg_connection *nc)//当可以发送数据出去的时候
{
	int n = 0;
 ...
 */
MG_INTERNAL struct mg_connection *mg_do_connect(struct mg_connection *nc, int proto, union socket_address *sa)//这边是真正的进行tcp和udp连接的地方
{
...
#endif
	{
		mg_call(nc, NULL, nc->user_data, MG_EV_CONNECT, &err);//这个是进行连接上以后的消息通知？这个应该是调用事件处理函数
	}
}
...
struct mg_connection *mg_connect_opt(struct mg_mgr *mgr, const char *address,
	MG_CB(mg_event_handler_t callback, void *user_data), struct mg_connect_opts opts)//callback是由用户定义并传进来的
{
	struct mg_connection *nc = NULL;
	int proto, rc;
	struct mg_add_sock_opts add_sock_opts;//添加socket的相关参数
	char host[MG_MAX_HOST_LEN];

	MG_COPY_COMMON_CONNECTION_OPTIONS(&add_sock_opts, &opts);

	if ((nc = mg_create_connection(mgr, callback, add_sock_opts)) == NULL)//建立连接
	{
		return NULL;
	}
	...
	MG_CB(mg_event_handler_t event_handler, void *user_data))
{
	...
	memset(&opts, 0, sizeof(opts));
	return mg_bind_opt(srv, address, MG_CB(event_handler, user_data), opts);//绑定的时候，这个回调处理函数是设置为OnHttpWebsocketEvent
}

struct mg_connection *mg_bind_opt(struct mg_mgr *mgr, const char *address,
	MG_CB(mg_event_handler_t callback, void *user_data), struct mg_bind_opts opts)
{
	union socket_address sa;
	struct mg_connection *nc = NULL;
	int proto, rc;
	...
	if (mg_parse_address(address, &sa, &proto, host, sizeof(host)) <= 0)//解析地址字符串
	{
		MG_SET_PTRPTR(opts.error_string, "cannot parse address");
		return NULL;
	}
	...
	{
		rc = nc->iface->vtable->listen_tcp(nc, &nc->sa);//开始监听
	}
	if (rc != 0)
	{
...
{
	struct ctl_msg ctl_msg;//这是一个控制消息
	...
	if (mgr->ctl[0] != INVALID_SOCKET && data != NULL &&len < sizeof(ctl_msg.message))
	{
		...
		dummy = MG_SEND_FUNC(mgr->ctl[0], (char *)&ctl_msg,
			offsetof(struct ctl_msg, message) + len, 0);//这边发送数据连函数指针都发送出去了
...
static int parse_net(const char *spec, uint32_t *net, uint32_t *mask)//对网络地址进行解析
{
...
void mg_if_get_conn_addr(struct mg_connection *nc, int remote, union socket_address *sa)//获取连接地址
{
	nc->iface->vtable->get_conn_addr(nc, remote, sa);
}

struct mg_connection *mg_add_sock_opt(struct mg_mgr *s, sock_t sock,
	MG_CB(mg_event_handler_t callback, void *user_data), struct mg_add_sock_opts opts)
{
#if MG_ENABLE_CALLBACK_USERDATA
	opts.user_data = user_data;
#endif
...
struct mg_connection *mg_add_sock(struct mg_mgr *s, sock_t sock,
	MG_CB(mg_event_handler_t callback, void *user_data))//添加一个socket，就是添加一个连接conn
{
	struct mg_add_sock_opts opts;
	memset(&opts, 0, sizeof(opts));
...
extern const struct mg_iface_vtable mg_default_iface_vtable;//这个是默认的处理连接
...
int mg_num_ifaces = (int) (sizeof(mg_ifaces) / sizeof(mg_ifaces[0]));//这边已经对处理函数的定义做了处理

struct mg_iface *mg_if_create_iface(const struct mg_iface_vtable *vtable, struct mg_mgr *mgr)//创建mg_iface，每个connection都有一个
{
...
double mg_mgr_min_timer(const struct mg_mgr *mgr)//获取最小的定时器
{
	double min_timer = 0;
	struct mg_connection *nc;
...
static time_t mg_null_if_poll(struct mg_iface *iface, int timeout_ms)//poll这个函数是用来对某个manager下面的所有connection进行网络事件的查询
{
...
void mg_socket_if_connect_tcp(struct mg_connection *nc, const union socket_address *sa)//真正的tcp连接
{
	int rc, proto = 0;
...
{
	int n = (int)MG_RECV_FUNC(nc->sock, buf, len, 0);//如果recv函数在等待协议接收数据时网络中断了，那么它返回0。
	if (n == 0)
	{
...
static int mg_accept_conn(struct mg_connection *lc)
{
	...
	sock_t sock = accept(lc->sock, &sa.sa, &sa_len);//接受一个socket
	if (sock == INVALID_SOCKET)
	{
		if (mg_is_error())
		{
	...
	nc = mg_if_accept_new_conn(lc);//创建一个connection
	if (nc == NULL)
	{
		closesocket(sock);
		return 0;
...
void mg_mgr_handle_conn(struct mg_connection *nc, int fd_flags, double now)//处理连接
{
	...
	if (!mg_if_poll(nc, now)) return;//这边调用的是对事件的具体处理
	//下面的代码都是对相关事件的callback。
	if (nc->flags & MG_F_CONNECTING)//如果当前的连接正在连接 但是还没有成功 
	{
		if (fd_flags != 0)
		{
			int err = 0;
	...
	if (fd_flags & _MG_F_FD_CAN_READ)
	{
		...
		else
		{
			...
			else//所有不是监听的状态都是跑到这边执行 就是因为这样 导致连接websocket的时候 服务端的socket没有发送任何数据，然后直接退出
			{
				mg_if_can_recv_cb(nc);//这边对flag进行了操作
			}
		}
	}
...
void mg_socket_if_init(struct mg_iface *iface)//创建报文的socketpair
{
	(void)iface;
	DBG(("%p using select()", iface->mgr));
#if MG_ENABLE_BROADCAST
	mg_socketpair(iface->mgr->ctl, SOCK_DGRAM);//创建socketpair
#endif
}
...
void mg_add_to_set(sock_t sock, fd_set *set, sock_t *max_fd)//用于在文件描述符集合中增加一个新的文件描述符
{
	if (sock != INVALID_SOCKET
#ifdef __unix__
		...
		)
	{
		...
		{
			*max_fd = sock;//设置最大的socket
		}
	}
}

time_t mg_socket_if_poll(struct mg_iface *iface, int timeout_ms)//这个是查看socket是否有事件来
{
	struct mg_mgr *mgr = iface->mgr;//通过这行函数是不是可以算出poll都是以iface为单位的，也就是一个mg_iface对应一个socket的所有的网络事件处理
	double now = mg_time();
	double min_timer;
	struct mg_connection *nc, *tmp;
	struct timeval tv;
	 ...
	 */
	min_timer = 0;
	for (nc = mgr->active_connections, num_fds = 0; nc != NULL; nc = tmp)//将当前所有的活动连接的网络事件添加到需要监控的事件里面
	{
		tmp = nc->next;
		...
		if (nc->sock != INVALID_SOCKET)
		{
			...
			{
				mg_add_to_set(nc->sock, &read_set, &max_fd);//添加读数据信号
			}

			if (((nc->flags & MG_F_CONNECTING) && !(nc->flags & MG_F_WANT_READ)) ||
				(nc->send_mbuf.len > 0 && !(nc->flags & MG_F_CONNECTING)))
			{
				mg_add_to_set(nc->sock, &write_set, &max_fd);//添加写数据信号
				mg_add_to_set(nc->sock, &err_set, &max_fd);
			}
		}

		if (nc->ev_timer_time > 0)//这是定时器事件来的时间频率 因为可能有多个连接，这边取频率最小的时间进行定义
		{
			if (num_timers == 0 || nc->ev_timer_time < min_timer)
			{
				min_timer = nc->ev_timer_time;//获取最小的定时器时间
			}
			num_timers++;
		}
	}
	...
	for (nc = mgr->active_connections; nc != NULL; nc = tmp)
	{
		...
		tmp = nc->next;
		mg_mgr_handle_conn(nc, fd_flags, now);//处理每一个连接的相关事情
	}
...
{
	union socket_address sa, sa2;//这边的这个socket_address是一个封装后的结果
	sock_t sock;
	...
	if ((sock = socket(AF_INET, sock_type, 0)) == INVALID_SOCKET)//分配socket
	{
	}
	else if (bind(sock, &sa.sa, len) != 0)//为socket绑定地址
	{
	}
	else if (sock_type == SOCK_STREAM && listen(sock, 1) != 0)//监听出错就进入{}内执行，否则不进入
	{
	}
	...
	else if (sock_type == SOCK_STREAM && connect(sp[0], &sa.sa, len) != 0)//连接出错进入{}内执行
	{
	}
	...
	else if ((sp[1] = (sock_type == SOCK_DGRAM ? sock : mg_socketpair_accept(sock, &sa, len))) == INVALID_SOCKET)//接受到了sp[0]的连接请求
	{
	}
	else
	{
...
int mg_parse_uri(const struct mg_str uri, struct mg_str *scheme, struct mg_str *user_info, struct mg_str *host,
	unsigned int *port, struct mg_str *path, struct mg_str *query, struct mg_str *fragment)
{
...
#if MG_ENABLE_HTTP  //这边定义了所有http相关
...
struct mg_http_proto_data_cgi
{
	struct mg_connection *cgi_nc;//cgi的协议相关数据是一个connection
};
#endif
...
struct mg_http_endpoint //这是一个http端点的链表
{
...
enum mg_http_multipart_stream_state//这个是http multipart的数据接收的数据状态机
{
	MPS_BEGIN,
...
{
	const char *boundary;//这个是多段数据之间的分隔符
	int boundary_len;
	const char *var_name;
...
struct mg_reverse_proxy_data //反向代理相关
{
...
struct mg_ws_proto_data
{
	 ...
	 * defragmented data.
	 */
	size_t reass_len;//记录了读取数据的大小，因为读取数据的时候可能要读取多次
};

struct mg_http_proto_data//这个应该是一个比较重要的类 与http相关的东西
{
#if MG_ENABLE_FILESYSTEM
...
static struct mg_http_proto_data *mg_http_get_proto_data(struct mg_connection *c)
{
	...
	{
		c->proto_data = MG_CALLOC(1, sizeof(struct mg_http_proto_data));//每个connection都有一个proto_data,这个是当前连接的协议的相关东西
		c->proto_data_destructor = mg_http_conn_destructor;//连接的析构函数
	}
...
static struct mg_str mg_get_mime_type(const char *path, const char *dflt,
	const struct mg_serve_http_opts *opts)//这个应该是分析当前http请求的Content_type 这个里面的mg_serve_http_opts是表示服务器里面的一些东西
{
	...
	while ((overrides = mg_next_comma_list_entry(overrides, &k, &v)) != NULL)//这个函数的意思是从".txt=text/plain; charset=utf-8,.c=text/plain"
	{
		ext = path + (path_len - k.len);
		if (path_len > k.len && mg_vcasecmp(&k, ext) == 0)//就是不知道这个path指的是什么
		{
			return v;
		}
	}
 ...
 */
static int mg_http_get_request_len(const char *s, int buf_len)//判断http请求的长度
{
	...
	for (i = 0; i < buf_len; i++)
	{
		...
		{
			return i + 2;//这个判断也合法吗,这个应该是对那种以\n\n作为http首部和荷载之间的分隔符，而不是\r\n作为分隔符的情况
		}
...
static const char *mg_http_parse_headers(const char *s, const char *end,
	int len, struct http_message *req)//这个函数是分析http请求头，s代表请求头的开始，end是请求头的结束，同时也是请求内容的开始
{
	int i = 0;
	while (i < (int)ARRAY_SIZE(req->header_names) - 1)
	{
		...
		s = mg_skip(s, end, ": ", k);//这个读到http请求的名字
		s = mg_skip(s, end, "\r\n", v);//这个读到http请求的值

		while (v->len > 0 && v->p[v->len - 1] == ' ')//这个是去掉value值得两边的空格
		{
		 ...
		 * behaviour
		 */
		if (k->len != 0 && v->len == 0)//header value等于0的情况
		{
			continue;
		}
		...
		{
			req->body.len = (size_t)to64(v->p);//获取http内容的长度
			req->message.len = len + req->body.len;//获取整个信息的长度
		}
...
int mg_parse_http(const char *s, int n, struct http_message *hm, int is_req)//对缓冲区的数据进行http格式的分析
{
	const char *end, *qs;
	int len = mg_http_get_request_len(s, n);//分析请求的长度，如果数据不完整，那就直接返回
	...
	hm->message.p = s;
	hm->body.p = s + len;
	hm->message.len = hm->body.len = (size_t)~0;//这边对两个相关的长度都没有设置为正确的
	end = s + len;
	...
	if (is_req)//如果是http请求
	{
		...
		s = mg_skip(s, end, "\r\n", &hm->proto);//这个得出的是类似于http/1.1这样的字符串
		if (hm->uri.p <= hm->method.p || hm->proto.p <= hm->uri.p) return -1;

		/* If URI contains '?' character, initialize query_string */
		if ((qs = (char *)memchr(hm->uri.p, '?', hm->uri.len)) != NULL)//这是http请求的参数
		{
...
struct mg_str *mg_get_http_header(struct http_message *hm, const char *name)//在http消息中查找是否有name这个key对应的value
{
...
#if MG_ENABLE_FILESYSTEM
static void mg_http_transfer_file_data(struct mg_connection *nc) //http发送具体的文件
{
	struct mg_http_proto_data *pd = mg_http_get_proto_data(nc);
	char buf[MG_MAX_HTTP_SEND_MBUF];
	size_t n = 0, to_read = 0, left = (size_t)(pd->file.cl - pd->file.sent);//获取当前还有多少数据没有发送

	if (pd->file.type == DATA_FILE)
	{
		struct mbuf *io = &nc->send_mbuf;
		if (io->len >= MG_MAX_HTTP_SEND_MBUF) //如果当前connection里面的发送缓存数据比buf的所有空间都大，
		{
			to_read = 0;//这边我知道了，如果connection里面的发送缓存已经有这么多数据在等待，那这次就不读取
		}
		else
		{
			to_read = MG_MAX_HTTP_SEND_MBUF - io->len;//这次拷贝的数据的数量
		}
		if (to_read > left)
		{
			to_read = left;
		...
		if (to_read > 0)
		{
			n = mg_fread(buf, 1, to_read, pd->file.fp);//读取一定量的数据并发送出去
			if (n > 0)
			{
				mg_send(nc, buf, n);
				pd->file.sent += n;
		...
		if (pd->file.sent >= pd->file.cl)//数据已经发送完成 
		{
	...
	else if (pd->file.type == DATA_PUT)//这个data_put的意思是什么 从connection的接受缓存里读取数据到文件里保存 这个表示是客户端上传文件？？
	{
	...
	size_t blen)//对http中chunk的处理
{
	...
	for (i = body_len; (n = mg_http_parse_chunk(buf + i, blen - i, &data, &data_len)) > 0; i += n)
	{
...
#else  /* !__XTENSA__ */
void mg_http_handler(struct mg_connection *nc, int ev,
	void *ev_data MG_UD_ARG(void *user_data)) //http的处理函数 比较重要的处理函数 也就是说有任何的事件都是先调用协议的处理函数，然后再调用用户自定义的函数
{
	...
	struct mbuf *io = &nc->recv_mbuf;
	int req_len;
	const int is_req = (nc->listener != NULL);//判断是客户端还是服务端
#if MG_ENABLE_HTTP_WEBSOCKET
	struct mg_str *vec;
#endif
	if (ev == MG_EV_CLOSE)
	{
		...
		if (pd->cgi.cgi_nc != NULL)
		{
			...
			pd->cgi.cgi_nc->flags |= MG_F_CLOSE_IMMEDIATELY;//关闭这个连接
		}
#endif
#if MG_ENABLE_HTTP_STREAMING_MULTIPART
	...
	mg_call(nc, nc->handler, nc->user_data, ev, ev_data);//这边是调用用户自定义的handler

	if (ev == MG_EV_RECV)//http的接收数据的处理
	{
		struct mg_str *s;
		pd->rcvd += *(int *)ev_data;//这边将接收到的数据的数量增加一些
	...
	again:
		req_len = mg_parse_http(io->buf, io->len, hm, is_req);//进行http数据的分析
...
#if MG_ENABLE_HTTP_WEBSOCKET
		else if (nc->listener == NULL && mg_get_http_header(hm, "Sec-WebSocket-Accept"))//Sec-WebSocket-Accept是服务器对客户端websocket的请求的回复
		{
		...
		else if (nc->listener != NULL && (vec = mg_get_http_header(hm, "Sec-WebSocket-Key")) != NULL)//这个是websocket端对用户websocket的请求的处理
		{
			struct mg_http_endpoint *ep;
...
static size_t mg_get_line_len(const char *buf, size_t buf_len)
{
	...
	return len == buf_len ? 0 : len + 1;//获取一行的长度
}
...
void mg_set_protocol_http_websocket(struct mg_connection *nc)//设置httpwebsocket的事件处理函数
{
	nc->proto_handler = mg_http_handler;//设置连接的相关处理函数
}
...
void mg_send_response_line_s(struct mg_connection *nc, int status_code,
	const struct mg_str extra_headers)//发送response的头
{
...
int mg_url_decode(const char *src, int src_len, char *dst, int dst_len,
	int is_form_url_encoded)//将url中的字符解码，这个是base64的编码？？
{
	int i, j, a, b;
...
int mg_get_http_var(const struct mg_str *buf, const char *name, char *dst,
	size_t dst_len) //从buf的数据中找到name参数对应的值
{
	const char *p, *e, *s;
	size_t name_len;
	int len;
...
void mg_send_http_chunk(struct mg_connection *nc, const char *buf, size_t len)//这个是对chunk数据的发送
{
	char chunk_size[50];
	int n;
...
void mg_printf_http_chunk(struct mg_connection *nc, const char *fmt, ...)//chunk的输出数据
{
	char mem[MG_VPRINTF_BUFFER_SIZE], *buf = mem;
	int len;
	va_list ap;
...
static void mg_http_parse_header_internal(struct mg_str *hdr, const char *var_name,
struct altbuf *ab)
{
...
int mg_parse_http_basic_auth(struct mg_str *hdr, char *user, size_t user_len,
	char *pass, size_t pass_len)//从请求报文中解析出用户名密码
{
	char *buf = NULL;
	char fmt[64];
	int res = 0;
...
int mg_http_create_digest_auth_header(char *buf, size_t buf_len,
	...
	const char *passwd, const char *nonce) //建立digest类型的验证头信息
{
...
MG_INTERNAL int mg_uri_to_local_path(struct http_message *hm, const struct mg_serve_http_opts *opts,
	char **local_path, struct mg_str *remainder)
{
	int ok = 1;
	const char *cp = hm->uri.p, *cp_end = hm->uri.p + hm->uri.len;//cp是http里面的url除了ip和端口的部分,cp_end是url地址的结尾
	struct mg_str root = { NULL, 0 };
	const char *file_uri_start = cp;//文件地址的开始，例如一个http://127.0.0.1:7999/web/index.html，cp指的是7999里面的第一个"/"
	*local_path = NULL;
	remainder->p = NULL;
	remainder->len = 0;
	...
	{ /* 1. Determine which root to use. */
		...
		const char *rewrites = "";
#endif
		struct mg_str *hh = mg_get_http_header(hm, "Host");//从http请求头里获取host的地址
		struct mg_str a, b;
  ...
  { /* 2. Find where in the canonical URI path the local path ends. */
	  const char *u = file_uri_start + 1;//跳过前面的'/'字符
	  char *lp = (char *)MG_MALLOC(root.len + hm->uri.len + 1);//获取本地的文件路径
...
void mg_http_send_digest_auth_request(struct mg_connection *c,
	const char *domain)//表示cookie的作用域，Domain=.169it.com
{
	mg_printf(c,
...
static int mg_is_creation_request(const struct http_message *hm)//创建集合（文件夹）
{
	return mg_vcmp(&hm->method, "MKCOL") == 0 || mg_vcmp(&hm->method, "PUT") == 0;
}

MG_INTERNAL void mg_send_http_file(struct mg_connection *nc, char *path, const struct mg_str *path_info,
struct http_message *hm,
struct mg_serve_http_opts *opts)
{
...
void mg_serve_http(struct mg_connection *nc, struct http_message *hm,struct mg_serve_http_opts opts)
{
	...
	uint32_t remote_ip = ntohl(*(uint32_t *)&nc->sa.sin.sin_addr);//获取ip
	...
	if (!mg_normalize_uri_path(&hm->uri, &hm->uri))//将地址中的.和..替换为本地的地址
	{
		mg_http_send_error(nc, 400, NULL);
		return;
	}
	if (mg_uri_to_local_path(hm, &opts, &path, &path_info) == 0)//将地址替换为本地地址
	{
		mg_http_send_error(nc, 404, NULL);
		return;
	}
...
struct mg_connection *mg_connect_http_base(struct mg_mgr *mgr, MG_CB(mg_event_handler_t ev_handler, void *user_data),
    struct mg_connect_opts opts, const char *scheme1, const char *scheme2,
	const char *scheme_ssl1, const char *scheme_ssl2, const char *url,
    struct mg_str *path, struct mg_str *user_info, struct mg_str *host)//这个是http基础连接的函数
{
	struct mg_connection *nc = NULL;
	...
	{
		mg_set_protocol_http_websocket(nc);//这边调用这个函数是因为http和websocket都是http相关的
	}
	...
	size_t *data_len)
{
	...
	if ((shl = mg_http_get_request_len(buf, buf_len)) <= 0) return 0;//获取http请求的长度
	hl = shl;
...
void mg_register_http_endpoint_opt(struct mg_connection *nc,
	const char *uri_path,
	mg_event_handler_t handler, struct mg_http_endpoint_opts opts)
{
...
static void *mg_push_to_stdin(void *arg)//这个是将客户端发送来的信息写入到服务器的控制台？？？？
{
		...
		(n = recv(tp->s, buf, sizeof(buf), 0)) > 0)
	{
		...
		{
			if (!WriteFile(tp->hPipe, buf + sent, n - sent, &k, 0)) stop = 1;//这个是向管道写数据，那管道里的另一端是什么。哦，对了，是stdin
		}
	}
	...
	const char *dir, sock_t sock)
{
	...
	HANDLE a[2], b[2], me = GetCurrentProcess();//获取当前进程的一个伪句柄
	...
	CreatePipe(&a[0], &a[1], NULL, 0);//管 道（Pipe）实际是用于进程间通信的一段共享内存，创建管道的进程称为管道服务器，连接到一个管道的进程为管道客户机。一个进程在向管道写入数据后，另 一进程就可以从管道的另一端将其读取出来。匿名管道（Anonymous Pipes）是在父进程和子进程间单向传输数据的一种未命名的管道，只能在本地计算机中使用，而不可用于网络间的通信。
	CreatePipe(&b[0], &b[1], NULL, 0);//第一个参数是读管道，第二个参数是写管道
	DuplicateHandle(me, a[0], me, &si.hStdInput, 0, TRUE, flags);//进程间传递句柄，这边是从a[0]读，也就是创建起来的进程从a[0]读取数据
	DuplicateHandle(me, b[1], me, &si.hStdOutput, 0, TRUE, flags);//向b[1]输出数据。也就是创建起来的进程往b[1]写数据
	...
	{
		mg_spawn_stdio_thread(sock, a[1], mg_push_to_stdin);//在创建一个进程之后，再创建线程。
...
static char *mg_addenv(struct mg_cgi_env_block *block, const char *fmt, ...)
{
	...
	space = sizeof(block->buf) - (block->len + 2);//这边为什么要加2
	if (space > 0)
	{
...
MG_INTERNAL void mg_handle_cgi(struct mg_connection *nc, const char *prog,
	const struct mg_str *path_info,
	const struct http_message *hm,
	const struct mg_serve_http_opts *opts)//这个是具体的处理cgi指令的地方
{
	struct mg_cgi_env_block blk;
	...
	if (!mg_socketpair(fds, SOCK_STREAM))//创建了一对socket以进行socket通信
	{
		nc->flags |= MG_F_CLOSE_IMMEDIATELY;
		return;
	}
...
MG_INTERNAL int mg_is_dav_request(const struct mg_str *s) //底下这些都是dav相关的命令
{
...
static int mg_deliver_websocket_data(struct mg_connection *nc)//这个函数只是将接收到的数据进行整理并发送给用户自定义的函数去处理
{
	/* Using unsigned char *, cause of integer arithmetic below */
	uint64_t i, data_len = 0, frame_len = 0, new_data_len = nc->recv_mbuf.len,
		len, mask_len = 0, header_len = 0;
	struct mg_ws_proto_data *wsd = mg_ws_get_proto_data(nc);//获取协议相关的数据
	unsigned char *new_data = (unsigned char *)nc->recv_mbuf.buf,
		*e = (unsigned char *)nc->recv_mbuf.buf + nc->recv_mbuf.len;//new_data是接受到的数据的开头，e是结束到的数据的结尾
	uint8_t flags;
	int ok, reass;

	if (wsd->reass_len > 0)//表示之前已经接收到了有关数据帧 只是还没有完整
	{
		/*
	...
	flags = new_data[0];//这个是新的一帧数据的第一个字节

	reass = new_data_len > 0 && mg_is_ws_fragment(flags) &&
		!(nc->flags & MG_F_WEBSOCKET_NO_DEFRAG);//MG_F_WEBSOCKET_NO_DEFRAG:如果用户想关闭websocket的自动帧碎片整理功能，则由用户设置此标记
	...
	if (ok)
	{
		...
		if (reass)
		{
			...
			if (mg_is_ws_first_fragment(flags))//是数据报文的第一帧
			{
				/*
			...
			if (flags & FLAGS_MASK_FIN)//这个判断是否是最后一帧数据
			{
				...
				wsd->reass_len = 0;//因为这边已经收到了最后一帧的数据

				/* Pass reassembled message to the client code. */
				mg_handle_incoming_websocket_frame(nc, &wsm);
				mbuf_remove(&nc->recv_mbuf, cleanup_len); /* Cleanup frame */ //清除缓存
			}
		}
		else
		{
			 ...
			 */
			cleanup_len = (size_t)frame_len;//这边的意思是接收到了一个完整的数据帧
			...
			mg_handle_incoming_websocket_frame(nc, &wsm);//这边是交给用户自定义的函数进行处理
...
static void mg_send_ws_header(struct mg_connection *nc, int op, size_t len, struct ws_mask_ctx *ctx)//op是指websocket的帧数据类型
{
	int header_len;
	unsigned char header[10];//websocket头部的前面十个字节
...
static void mg_ws_mask_frame(struct mbuf *mbuf, struct ws_mask_ctx *ctx)
{
	size_t i;
	if (ctx->pos == 0) return;
	for (i = 0; i < (mbuf->len - ctx->pos); i++)//对发送的数据进行mask操作
	{
...
void mg_send_websocket_frame(struct mg_connection *nc, int op, const void *data, size_t len)
{
	...
	mg_ws_mask_frame(&nc->send_mbuf, &ctx);//这个是什么意思

	if (op == WEBSOCKET_OP_CLOSE)
	{
		nc->flags |= MG_F_SEND_AND_CLOSE;//对连接的状态进行设置
	}
}

void mg_send_websocket_framev(struct mg_connection *nc, int op,
	const struct mg_str *strv, int strvcnt)//发送多个字符串
{
	struct ws_mask_ctx ctx;
	int i;
	int len = 0;
...
MG_INTERNAL void mg_ws_handler(struct mg_connection *nc, int ev, void *ev_data MG_UD_ARG(void *user_data))//websocket的数据处理
{
	mg_call(nc, nc->handler, nc->user_data, ev, ev_data);//这个调用的是connection里面的handler，在上面我们知道这个是用户定义的handler
...
MG_INTERNAL void mg_ws_handshake(struct mg_connection *nc,
	const struct mg_str *key, struct http_message *hm)//这个是websocket服务端在接收到客户端的websocket的握手请求的时候的处理
{
...
void mg_send_websocket_handshake3v(struct mg_connection *nc,
	...
	const struct mg_str pass)//websocket的http的建立
{
	struct mbuf auth;
	char key[25];
	uint32_t nonce[4];
...
struct mg_connection *mg_connect_ws_opt(struct mg_mgr *mgr, MG_CB(mg_event_handler_t ev_handler, void *user_data),
	...
	const char *extra_headers)
{
	...
	if (nc != NULL)
	{
		mg_send_websocket_handshake3v(nc, path, host, mg_mk_str(protocol),
			...
			null_str);//在http连接建立以后 建立websocket的连接
	}
	return nc;
}

struct mg_connection *mg_connect_ws(struct mg_mgr *mgr, MG_CB(mg_event_handler_t ev_handler, void *user_data),
	const char *url, const char *protocol, const char *extra_headers)
{
	struct mg_connect_opts opts;//这边的mg_connect_opts是连接的数据结构
	memset(&opts, 0, sizeof(opts));
...
{
	v->p = s;
	while (s < end && strchr(delims, *(unsigned char *)s) == NULL) s++;//这个只是读取第一次出现delims这个字符之前的字符串
	v->len = s - v->p;
...
{
#if defined(_WIN32) && !defined(WINCE)
	(void)SetHandleInformation((HANDLE)sock, HANDLE_FLAG_INHERIT, 0);//这个是设置socket可以继承？
#elif defined(__unix__)
...
static int mg_hexdump_n(const void *buf, int len, char *dst, int dst_len, int offset)
{
	...
	char ascii[17] = "";//字符数组的赋值，会自动在末尾添加'\0'字符
	int i, idx, n = 0;
...
MG_INTERNAL int parse_mqtt(struct mbuf *io, struct mg_mqtt_message *mm)
{
	...
	if (io->len < 2) return MG_MQTT_ERROR_INCOMPLETE_MSG;//mqtt的固定头就有两个字节
	header = io->buf[0];
	cmd = header >> 4;
	...
	while ((size_t)(p - io->buf) < io->len)
	{
		...
		len_len++;
		if (!(lc & 0x80)) break;//当为true，就是最后一位为0时，表示长度不够需要两个字节表示
...
static void mqtt_handler(struct mg_connection *nc, int ev, void *ev_data MG_UD_ARG(void *user_data))//对mqtt数据进行处理，这个是系统定义的handler
{
	struct mbuf *io = &nc->recv_mbuf;//这个应该是mqtt客户端的事件处理逻辑
	...
	case MG_EV_POLL://这个事件是需要定时轮询的事件
	{
...
int mg_mqtt_match_topic_expression(struct mg_str exp, struct mg_str topic)
{
	...
	if (memchr(exp.p, '#', exp.len))//从exp.p所指内存区域的前exp.len个字节查找字符'#'
	{
...
void mg_set_protocol_mqtt(struct mg_connection *nc)//设置当前的连接的协议
{
...
static void mg_send_mqtt_header(struct mg_connection *nc, uint8_t cmd, uint8_t flags, size_t len)
{
	struct mg_mqtt_proto_data *pd = (struct mg_mqtt_proto_data *) nc->proto_data;
...
void mg_send_mqtt_handshake_opt(struct mg_connection *nc, const char *client_id, struct mg_send_mqtt_handshake_opts opts)//这个是连接到mqtt服务器
{
	struct mg_mqtt_proto_data *pd = (struct mg_mqtt_proto_data *) nc->proto_data;
	...
	if (client_id != NULL)
	{
		id_len = strlen(client_id);//获取握手mqtt包中的client_id的长度
	}

	total_len = 7 + 1 + 2 + 2 + id_len;//这边的这个长度是代表哪些数据
	...
	{
		opts.keep_alive = 60;//这是一个默认值，是60秒
	}
	...
	mg_send(nc, "\00\04MQTT\04", 7);//可变头部的协议相关
	mg_send(nc, &opts.flags, 1);//可变头部的连接标志相关

	netbytes = htons(opts.keep_alive);//保活的定时器相关
	mg_send(nc, &netbytes, 2);

	netbytes = htons(id_len);
	mg_send(nc, &netbytes, 2);
	mg_send(nc, client_id, id_len);//客户端ID相关

	if (opts.flags & MG_MQTT_HAS_WILL)//will相关
	{
	...
	if (opts.flags & MG_MQTT_HAS_USER_NAME)//用户名密码相关
	{
...
void mg_mqtt_publish(struct mg_connection *nc, const char *topic, uint16_t message_id, int flags, const void *data, size_t len)//发布消息
{
	uint16_t netbytes;
	...
	mg_send_mqtt_header(nc, MG_MQTT_CMD_PUBLISH, flags, total_len); //MQTT固定头

	netbytes = htons(topic_len);
	mg_send(nc, &netbytes, 2); //MQTT可变头
	...
	mg_send(nc, data, len);//Payload消息体
}

void mg_mqtt_subscribe(struct mg_connection *nc, const struct mg_mqtt_topic_expression *topics,
	size_t topics_len, uint16_t message_id)//这边的topics_len是指有多少个topic，而不是指topic里面字符串的长度
{
	uint16_t netbytes;
	size_t i;
	uint16_t topic_len;
	...
	netbytes = htons(message_id);
	mg_send(nc, (char *)&netbytes, 2);//这个是对可变头部的控制
...
void mg_mqtt_unsubscribe(struct mg_connection *nc, char **topics, size_t topics_len, uint16_t message_id)//取消订阅
{
	uint16_t netbytes;
	size_t i;
	uint16_t topic_len;
...
{
	uint8_t unused = 0;
	mg_send_mqtt_header(nc, MG_MQTT_CMD_CONNACK, 0, 2);//连接的确认
	mg_send(nc, &unused, 1);
...
void mg_mqtt_puback(struct mg_connection *nc, uint16_t message_id)//发布确认 QoS 1
{
	mg_send_mqtt_short_command(nc, MG_MQTT_CMD_PUBACK, message_id);//PUBACK报文是对QoS 1等级的PUBLISH报文的响应
}

void mg_mqtt_pubrec(struct mg_connection *nc, uint16_t message_id) //PUBREC报文是对QoS等级2的PUBLISH报文的响应。它是QoS 2等级协议交换的第二个报文
{
	mg_send_mqtt_short_command(nc, MG_MQTT_CMD_PUBREC, message_id); //PUBREC报文是对QoS等级2的PUBLISH报文的响应。
}

void mg_mqtt_pubrel(struct mg_connection *nc, uint16_t message_id)
{
	mg_send_mqtt_short_command(nc, MG_MQTT_CMD_PUBREL, message_id);//PUBREL报文是对PUBREC报文的响应。它是QoS 2等级协议交换的第三个报文
}

void mg_mqtt_pubcomp(struct mg_connection *nc, uint16_t message_id)
{
	mg_send_mqtt_short_command(nc, MG_MQTT_CMD_PUBCOMP, message_id);//PUBCOMP报文是对PUBREL报文的响应。它是QoS 2等级协议交换的第四个也是最后一个报文。
}
...
void mg_mqtt_suback(struct mg_connection *nc, uint8_t *qoss, size_t qoss_len, uint16_t message_id)
{
	...
	mg_send_mqtt_header(nc, MG_MQTT_CMD_SUBACK, MG_MQTT_QOS(1), 2 + qoss_len);//服务端发送SUBACK报文给客户端，用于确认它已收到并且正在处理SUBSCRIBE报文。

	netbytes = htons(message_id);//SUBACK报文包含一个返回码清单，它们指定了SUBSCRIBE请求的每个订阅被授予的最大QoS等级。
	mg_send(nc, &netbytes, 2);
...
{
	mg_send_mqtt_short_command(nc, MG_MQTT_CMD_UNSUBACK, message_id);//服务端发送UNSUBACK报文给客户端用于确认收到UNSUBSCRIBE报文
}

void mg_mqtt_ping(struct mg_connection *nc)
{
	mg_send_mqtt_header(nc, MG_MQTT_CMD_PINGREQ, 0, 0);//客户端发送PINGREQ报文给服务端的
}
...
static void mg_mqtt_broker_handle_subscribe(struct mg_connection *nc, struct mg_mqtt_message *msg) //mqtt_broker订阅的请求
{
	...
	for (pos = 0; (pos = mg_mqtt_next_subscribe_topic(msg, &topic, &qos, pos)) != -1;)
	{
		if (num_subs >= sizeof(MG_MQTT_MAX_SESSION_SUBSCRIPTIONS) ||
			(ss->num_subscriptions + num_subs >= MG_MQTT_MAX_SESSION_SUBSCRIPTIONS))
		{
			nc->flags |= MG_F_CLOSE_IMMEDIATELY;
			return;
		}
	...
	if (num_subs > 0)
	{
		te = (struct mg_mqtt_topic_expression *) MG_REALLOC(ss->subscriptions,
...
void mg_mqtt_broker(struct mg_connection *nc, int ev, void *data)//这个函数是对mqtt_broker的对客户端的相关处理问题
{
...
struct mg_dns_resource_record *mg_dns_next_record(struct mg_dns_message *msg, int query, struct mg_dns_resource_record *prev)
{
	...
	for (rr = (prev == NULL ? msg->answers : prev + 1); rr - msg->answers < msg->num_answers; rr++)
	{
		if (rr->rtype == query)
		{
			return rr;
...
struct mg_coap_option *mg_coap_add_option(struct mg_coap_message *cm,
	...
	size_t len) //value是指向的这个option内容的缓存
{
	...
	else 
	{
		 ...
		 */
		if (cm->optiomg_tail->number <= new_option->number) //这个按照升序排列
		{
		...
		else 
		{
			...
			while (current_opt != NULL) //将option插入到正确的位置
			{
 ...
 * Helper function.
 */
static char *coap_parse_header(char *ptr, struct mbuf *io,struct mg_coap_message *cm) //这个是对固定头的分析
{
...
static char *coap_get_token(char *ptr, struct mbuf *io,struct mg_coap_message *cm)
{
	if (cm->token.len != 0)
	{
		if (ptr + cm->token.len > io->buf + io->len)//这边的这个ptr应该是指向io缓冲区的指针
		{
			cm->flags |= MG_COAP_NOT_ENOUGH_DATA;
			return NULL;
		} 
...
struct mg_coap_message *cm)//从这个函数开始是真正的获取option
{
	uint16_t prev_opt = 0;
	...
	while (ptr < io->buf + io->len && (uint8_t) *ptr != 0xFF)
	{
		 ...
		 * between them.
		 */
		option_delta += prev_opt;//刚开始prev_opt为0，这边是累计option_delta

		mg_coap_add_option(cm, option_delta, ptr, option_lenght);//添加option
 ...
 * Helper function.
 */
static size_t coap_get_ext_opt_size(uint32_t value) //计算当前的大小应该占用的缓存的大小
{
	int ret = 0;
 ...
 * Helper function.
 */
static int coap_split_opt(uint32_t value, uint8_t *base, uint16_t *ext) //用于往报文里写
{
	int ret = 0;
 ...
 * Helper function.
 */
static char *coap_add_opt_info(char *ptr, uint16_t val, size_t len)//这个函数的意思是在len长度的缓存中存放val这个值
{
...
uint32_t mg_coap_compose(struct mg_coap_message *cm, struct mbuf *io) 
{
	...
	res = coap_calculate_packet_size(cm, &packet_size);//计算整个包的大小
	if (res != 0) 
	{
		return res;
	}
	...
	prev_io_len = io->len;
	if (mbuf_append(io, NULL, packet_size) == 0) return MG_COAP_ERROR;//查看内存是否够用
	...
	*ptr = (cm->code_class << 5) | (cm->code_detail);
	...
	ptr = coap_add_uint16(ptr, cm->msg_id);//上面是固定头的填充

	if (cm->token.len != 0)
	{
		memcpy(ptr, cm->token.p, cm->token.len);
		ptr += cm->token.len;//token的填充
	}
	...
	while (opt != NULL) 
	{
		...
		size_t opt_delta_len =
			coap_split_opt(opt->number - prev_opt_number, &delta_base, &delta_ext);//获取delta这个数字占用的缓存的大小
		size_t opt_lenght_len =
			coap_split_opt((uint32_t) opt->value.len, &length_base, &length_ext);//获取length这个数字占用的缓存的大小
...
uint32_t mg_coap_send_message(struct mg_connection *nc,struct mg_coap_message *cm)//发送数据
{
...
int mg_set_protocol_coap(struct mg_connection *nc) 
{
	...
	nc->proto_handler = coap_handler;//这边是设置了协议的处理函数
